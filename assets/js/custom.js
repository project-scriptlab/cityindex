$(document).ready(function(){
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (JsonObject.success) {
					...
					$('.detailsContainer').html(JsonObject.view);
					$('#withdrawlDetailsModal').modal('show');
					$("[name='csrf_token_name']").val(JsonObject.hash);
				} else {
					error(JsonObject.message);
					$("[name='csrf_token_name']").val(JsonObject.hash);
	...
	$(document).on('click', '.activeInactive', function(e){
		e.preventDefault();
		var parent = jQuery(this),
		type = parent.attr('data-val'),
		invId = parent.attr('data-id');
		if (type == 1) {
			var textM = "Are you sre? You want to active this investment ("+invId+")."
		}else{
			var textM = "Once it's inactivated, you won't be able to generate payout reports for the same, make sure you have generated payout reports of this investment ("+invId+").";
		}
		$.confirm({
			title: 'Confirm!',
			content: textM,
			...
			buttons: {
				omg: {
					...
					btnClass: 'btn-dark',
					action: function () {
						window.location.href = site_url+'payout/chage-investment-status/'+invId+'/'+type;
					}
				},
				No: function(){
				}
