        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-7">
                            <h4 class="header-title">Monthly interest of <?= $monthName.', '.$year; ?></h4>
                        </div>
                        <div class="col-md-5">
                            <form action="<?= site_url('investors/monthly-interest') ?>" method="post">
                                <?= csrf_field(); ?>
                        ...
                        <table id="" class=" basicDatatable table table-centered table-striped dt-responsive nowrap w-100">
                            <thead>
                                <tr>
                                    ...
                                    <th>Interest% <i class="fa-solid fa-circle-question" data-toggle="tooltip" title="Interest Percentage"></i></th>
                                    <th>Interest Per Day <i class="fa-solid fa-circle-question" data-toggle="tooltip" title="Per Day Interest"></i></th>
                            ...
                            <tbody>
                                <?php if (!empty($interests)): $i = 1; ?>
                                    <?php foreach ($interests as $interest): $bank = unserialize($interest->bank_details); ?>
                                        <tr>
                                            <td><?= $i++; ?></td>
                                            <td><?= $interest->investment_id; ?> <i class="fa-solid fa-circle-info" tabindex="0" data-bs-toggle="popover" title="Bank Details" data-bs-content="<?= !empty($interest->bank_details)?$bank->bank_name.'<br />A/C No. : '.$bank->account_number.'<br />IFSC : '.$bank->ifsc_code:'NA'; ?>" data-bs-html="true"></i></td>
                                            <td><i class="fa-solid fa-indian-rupee-sign"></i> <?= number_format($interest->investment_amount, 2); ?></td>
                                            <td><?= $interest->interest; ?> <i class="fa-solid fa-percent"></i></td>
                                            <td><i class="fa-solid fa-indian-rupee-sign"></i> <?= number_format($interest->per_day_interest, 2) ?></td>
                                            <td><?= $interest->total_days; ?></td>
                                            <td><i class="fa-solid fa-indian-rupee-sign"></i> <?= number_format($interest->total_interest, 2) ?></td>
